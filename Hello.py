...
if not filtered_df.empty:
    ...
    st.metric(label="Total Sales", value=f"${total_sales:.3f}")
    st.metric(label="Total Profit", value=f"${total_profit:.3f}")
    st.metric(label="Overall Profit Margin (%)", value=f"{overall_profit_margin:.3f}%")
...
if not filtered_df.empty:
    ...
    st.metric(label="Overall Profit Margin vs Average", value=f"{delta_margin:.3f}%", delta=delta_margin)
